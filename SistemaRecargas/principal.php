<?php
  error_reporting(0);
  include 'conexion.php';
  session_start();
    $usuario = $_SESSION['username'];
    if(!isset($usuario)){
      header("location: index.php");
    }
//#################################################################
if(isset($_POST['pagar'])){
  $matri = $_POST['matri'];//obtener campo usuario
  $pass = $_POST['pass'];
  $const = $_POST['document']; 
  $q = "SELECT * FROM alumnos WHERE matricula = '$matri'";
  $consulta = mysqli_query($con, $q);//EJECUTAR CONSULTA
  $filas = mysqli_num_rows($consulta);
  $dato = $consulta->fetch_assoc();
  $sald_old = $dato['saldo'];
  if($filas > 0){
    $cal = "SELECT * FROM documentos WHERE id_doc = '$const'";
    $res = mysqli_query($con,$cal);
    while($r = $res->fetch_assoc()){
      $doc = $r['precio'];
    }
  $sald_new = $sald_old + $doc;
  $pago = "UPDATE alumnos SET saldo='$sald_new' WHERE matricula = '$matri'";
  $res = mysqli_query($con,$pago);
            echo "correcto";
            echo "<br>";
            echo "$sald_new";
  
  }else{
    echo "Datos incorrectos";
          
  }
          mysqli_close($con);//cerrar para evitar consumir recursos
}
//#################################################################

?>
<!DOCTYPE html>
<html lang="en">

<head>

  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <meta name="description" content="">
  <meta name="author" content="">

  <title>Recargas</title>

  <!-- Bootstrap core CSS -->
  <link href="vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">

  <!-- Custom fonts for this template -->
  <link href="https://fonts.googleapis.com/css?family=Saira+Extra+Condensed:500,700" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css?family=Muli:400,400i,800,800i" rel="stylesheet">
  <link href="vendor/fontawesome-free/css/all.min.css" rel="stylesheet">

  <!-- Custom styles for this template -->
  <link href="css/resume.min.css" rel="stylesheet">
 

</head>

<body id="page-top">

  <nav class="navbar navbar-expand-lg navbar-dark bg-info fixed-top" id="sideNav">
    <a class="navbar-brand js-scroll-trigger" href="#page-top">
      <span class="d-block d-lg-none">UPEG ADMIN</span>
      <span class="d-none d-lg-block">
        <img class="img-fluid img-profile rounded-circle mx-auto mb-2" src="img/profile.png" alt="">
      </span>
    </a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent"
      aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav">
        <li class="nav-item">
          <h3 class="text-white">
            <?php echo "$usuario"; ?></h3>

        </li>

        <li class="nav-item">
          <a class="nav-link js-scroll-trigger" href="salir.php">SALIR</a>
        </li>
      </ul>
    </div>
  </nav>

  <div class="container-fluid p-0">

    <section class="resume-section p-3 p-lg-5 align-items-center">
      <div class="w-100">
        <h1 class="mb-0">Recarga
          <span class="text-info">cde</span>
        </h1>
        <div class="subheading mb-5">universidad politécnica del estado de guerrero
        </div>
      </div>

      <div class="div">
        <table class="table">
          <thead>
            <tr>
              <th>#</th>
              <th>
                <H5>DOCUMENTO</H5>
              </th>
              <th>
                <H5>PRECIO</H5>
              </th>
            </tr>
          </thead>
          <tbody>
            <tr class="">
              <th scope="row">1</th>
              <td>
                <h6>CONSTANCIA</h6>
              </td>
              <td>
                <H6>$20</H6>
              </td>
            </tr>
            <tr>
              <th scope="row">2</th>
              <td>
                <H6>CARDEX</H6>
              </td>
              <td>
                <H6>$50</H6>
              </td>
            </tr>
          </tbody>
        </table>
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>" method="post">
          <div class="subheading mb-1 text-info">ingresar matrícula
            <input class="form-control" title="Ingresar Matricula (8 Digitos)" type="text" placeholder="Matrícula" name="matri" pattern="[0-9]{8}" required>
          </div>
          <div class="subheading mb-1 text-info">seleccionar documento
          </div>
          <select class="custom-select" name="document" required>
            <option value="">Open this select menu</option>
            <option value="1">CONSTANCIA</option>
            <option value="2">KARDEX</option>
          </select>
      </div>
      <div class="mt-5 text-right">
        <button type="submit" class="btn btn-primary" name="pagar">PAGAR</button>
      </div>
      </form>
    </section>

  </div>

  <!-- Bootstrap core JavaScript -->
  <script src="vendor/jquery/jquery.min.js"></script>
  <script src="vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

  <!-- Plugin JavaScript -->
  <script src="vendor/jquery-easing/jquery.easing.min.js"></script>

  <!-- Custom scripts for this template -->
  <script src="js/resume.min.js"></script>

</body>

</html>
